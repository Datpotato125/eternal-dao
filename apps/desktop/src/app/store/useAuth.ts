...
export const useAuth = create<AuthState>((set, get) => ({
  ...
  fetchCharacter: async () => {
    ...
    const [{ data: player }, { data: char }] = await Promise.all([
      ...
      supabase
        .from('characters')
        .select('id, server_id, realm_level, qi_current, qi_max, cultivation_rate, spirit_root, pvp_wins, pvp_losses, spirit_stones, cultivation_started_at, players!player_id(username, avatar_url)')
